...
void Drawer::zBufForModel(vector<Polygon> &polygons, vector<Vertex> &vertices, Eigen::Matrix4f &trans_mtr, size_t color, Platform *scene, size_t buf_w, size_t buf_h)
{
    /* ... */
    for (size_t cur_polygon = 0; cur_polygon < polygons.size(); cur_polygon++){
        ...
        for (int i = 0; i < 3; i++){
            ...
            cord_vec.row(i) << dotsArr[i].getXCoord(), dotsArr[i].getYCoord(), dotsArr[i].getZCoord(), 1;
        }
        cord_vec *= dottrans_mtr;
        /* ... */
        for (int cur_y = (y1 < 0) ? 0 : y1; cur_y < ((y2 >= (int)buf_h) ? (int)buf_h - 1 : y2); cur_y++) {
            double a_inc = 0;
            if (y1 != y2)
            ...
            for (int cur_x = x_a; cur_x <= x_b; cur_x++){
                ...
                if (curZ >= depthBuffer.at(cur_x).at(cur_y))
                {
                    ...
                    for (size_t i = 0; i < scene->getLightNum() && !visible; i++){
                        /* ... */
                        if (x < (int)shadowMap->size() && x >= 0 && y < (int)shadowMap->at(0).size() && y >= 0 && fabs(shadowMap->at(x).at(y) - newCoordinates(0, 2)) < 2) 
                            visible = 1;
                    }
