...
FileCameraLoader::FileCameraLoader(const std::string &name) {
...
std::shared_ptr<Object> FileCameraLoader::load(std::shared_ptr<BaseBuilder> src_builder) {
    ...
    builder->buildPosition(read_position());
    builder->buildDirection(read_direction());
...
std::shared_ptr<BaseLoader> FileCameraLoaderCreator::createLoader(const std::string &name) {
